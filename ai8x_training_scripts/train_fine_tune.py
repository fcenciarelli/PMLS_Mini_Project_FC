...
def main():
    ...
    if args.prototype:
        msglogger.info('Training the prototype')
        return train_proto(train_loader, test_loader, model, criterion, optimizer, scheduler, 
                           compression_scheduler, loggers=all_loggers, args=args)
    ...
    for epoch in range(start_epoch, ending_epoch):
        ...
        with collectors_context(activations_collectors["train"]) as collectors:
            ...
            if args.finetune:
                train_finetune(train_loader, model, criterion, optimizer, epoch, compression_scheduler,
                               loggers= all_loggers, args =args)

            elif args.finetuneNumpy:
                train_finetune_numpy(train_loader, model, criterion, optimizer, epoch, compression_scheduler,
                                    loggers= all_loggers, args = args)
            elif args.finetuneConv:
                train_finetune_numpy_conv(train_loader, model, criterion, optimizer, epoch, compression_scheduler,
                                    loggers= all_loggers, args = args)
...
def create_optimizer(model, args):
    ...
    if args.optimizer.lower() == 'sgd':
        ...
        if args.finetune:
            ...
            # model.conv1.op.weight.requires_grad = True
            # model.conv2.op.weight.requires_grad = True
            # model.conv3.op.weight.requires_grad = True
            model.conv4.op.weight.requires_grad = True
            model.fc.op.weight.requires_grad = True
            model.fc.op.bias.requires_grad = True
...
def get_validation_episode(val_class_samples, num_classes, num_support, num_query):
    while True:
        episode_classes = random.sample(list(val_class_samples.keys()), num_classes)
        ...
        for i, cls in enumerate(episode_classes):
            if len(val_class_samples[cls]) < num_support + num_query:
                continue  # Skip if not enough samples

            samples = random.sample(val_class_samples[cls], num_support + num_query)
            support_samples = samples[:num_support]
            query_samples = samples[num_support:]

            support_images.extend(support_samples)
            support_labels.extend([i] * num_support)
            query_images.extend(query_samples)
            query_labels.extend([i] * num_query)

        # Convert lists to tensors and yield
        yield torch.stack(support_images), torch.tensor(support_labels), torch.stack(query_images), torch.tensor(query_labels)


def validate(model, val_class_samples, num_classes, num_support, num_query, num_episodes):
    model.eval()
    total_loss = 0.0
    total_accuracy = 0.0

    val_episode_loader = get_validation_episode(val_class_samples, num_classes, num_support, num_query)

    with torch.no_grad():
        for episode in range(num_episodes):
            support_images, support_labels, query_images, query_labels = next(val_episode_loader)
            ...
            distances = cosine_similarity(query_features, prototypes)

            # Compute loss
            log_p_y = nn.functional.log_softmax(-distances, dim=1)
            loss = nn.functional.nll_loss(log_p_y, query_labels)
            total_loss += loss.item()

            # Compute accuracy
            y_hat = log_p_y.argmax(dim=1)
            accuracy = torch.eq(y_hat, query_labels).float().mean()
...
def train_proto(train_loader,test_loader, model, criterion, optimizer, scheduler, compression_scheduler, loggers, args):
    ...
    for inputs, target in train_loader:
        for i, label in enumerate(target):
            class_samples[label.item()].append(inputs[i])
    ...
    num_classes = 10  # For 5-way classification
    num_support = 10  # For 5-shot
    ...
    for episode in range(num_episodes):  # Define num_episodes as needed

        while True:
            episode_classes = random.sample(list(class_samples.keys()), num_classes)
            support_images, support_labels, query_images, query_labels = [], [], [], []
            valid_episode = True

            for i, cls in enumerate(episode_classes):
                if len(class_samples[cls]) < num_support + num_query:
                    valid_episode = False
                    break  # Break the inner loop

                samples = random.sample(class_samples[cls], num_support + num_query)
                support_samples, query_samples = samples[:num_support], samples[num_support:]
                support_images.extend(support_samples)
                support_labels.extend([i] * num_support)
                query_images.extend(query_samples)
                query_labels.extend([i] * num_query)
        ...
        if query_labels.ndim > 1 and query_labels.size(1) > 1:  # Check if one-hot encoded
        ...
        loss = nn.functional.nll_loss(log_p_y, query_labels)
        y_hat = log_p_y.argmax(dim=1)
        accuracy = torch.eq(y_hat, query_labels).float().mean().item()
        ...
        if episode % 2 == 0:  # Adjust logging frequency as needed
            current_lr = optimizer.param_groups[0]['lr']
            print(f'Episode {episode}, Loss: {loss.item():.4f}, Accuracy: {accuracy * 100:.2f}%, LR: {current_lr:.6f}')


        # validate_proto(model, val_class_samples, num_classes, num_support, num_query, num_episodes)
    return loss
...
def compute_prototypes(support_set, support_labels, num_classes):
    ...
    for c in range(num_classes):
        ...
        is_class = support_labels == c
        class_features = support_set[is_class]

        prototypes[c] = class_features.mean(0)
...
def gradient_softmax_cross_entropy(softmax_output, labels):

    grad = softmax_output.copy()
    grad[range(len(labels)), labels] -= 1
    grad = grad / len(labels)
    return grad


def gradient_fc(input_data, grad_output, weights, biases):

    grad_input = np.dot(grad_output, weights)
    grad_weights = np.dot(grad_output.T, input_data)
    grad_biases = np.sum(grad_output, axis=0)

    return grad_input, grad_weights, grad_biases


def gradient_conv2d(input_data, grad_output, kernel, bias):
    ...
    for i in range(input_height - kernel_height + 1):
        for j in range(input_width - kernel_width + 1):
            # Calculate gradient w.r.t. input data
            grad_input[:, i:i+kernel_height, j:j+kernel_width] += grad_output[:, i, j][:, None, None] * kernel

            # Calculate gradient w.r.t. kernel weights
            for n in range(batch_size):
                grad_kernel += grad_output[n, i, j] * input_data[n, i:i+kernel_height, j:j+kernel_width]

    # Sum of gradients over all images in the batch for bias
...
def train_finetune_numpy_conv(train_loader, model, criterion, optimizer, epoch,
                         compression_scheduler, loggers, args):
    
    original_fc = model.fc
    original_conv4 = model.conv4
    # model.conv4 = nn.Flatten
    model.fc = nn.Flatten()

    print(model)

    # for param in original_fc:
    #     print(param)
    ...
    for train_step, (inputs, target) in enumerate(train_loader):
        ...
        fc_weights_np = original_fc.op.weight.detach().cpu().numpy()
        fc_biases_np = original_fc.op.bias.detach().cpu().numpy()
        outputs = np.dot(features, fc_weights_np.T) + fc_biases_np
        softmax_output = softmax_impl(outputs)
        
        # Loss calculation
        loss = cross_entropy_loss(softmax_output, target.cpu().numpy())

        # Backward pass
        grad_output = gradient_softmax_cross_entropy(softmax_output, target.cpu().numpy())
        _, grad_weights, grad_biases = gradient_fc(features, grad_output, fc_weights_np, fc_biases_np)


        grad_output_conv4 = np.dot(grad_output, original_fc.op.weight.data.cpu().numpy())
        _, grad_kernel_conv4, grad_bias_conv4 = gradient_conv2d(conv4_input, grad_output_conv4, conv4_weights_np, conv4_biases_np)

        # Update weights and biases
        updated_weights = original_fc.op.weight.data.cpu().numpy() - learning_rate * grad_weights
        updated_biases = original_fc.op.bias.data.cpu().numpy() - learning_rate * grad_biases

        updated_weights_conv = original_conv4.op.weight.data.cpu().numpy() - learning_rate * grad_weights
        updated_biases_conv = original_conv4.op.bias.data.cpu().numpy() - learning_rate * grad_biases

        # Apply the updates to the model parameters
        original_fc.op.weight.data = torch.from_numpy(updated_weights).to(args.device)
        original_fc.op.bias.data = torch.from_numpy(updated_biases).to(args.device)




        losses[OBJECTIVE_LOSS_KEY].add(loss)
...
def train_finetune_numpy(train_loader, model, criterion, optimizer, epoch,
                         compression_scheduler, loggers, args):
...
def train_finetune(train_loader, model, criterion, optimizer, epoch,
          compression_scheduler, loggers, args):
    """Training loop for one epoch, just by finetuning the last FC layer."""
    losses = OrderedDict([(OVERALL_LOSS_KEY, tnt.AverageValueMeter()),
                          (OBJECTIVE_LOSS_KEY, tnt.AverageValueMeter())])
    ...
    if args.nas:
     ...
     # fine tuning only the FC

    # optimizer = torch.optim.SGD(model.fc.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    # print("Fully connected layer training status:")
    # for name, param in model.named_parameters():
    #     if name.startswith('fc') or name.startswith('conv4') or name.startswith('conv3') :
    #         print(f"{name}: requires_grad = {param.requires_grad}")
    #     else:
    #         if param.requires_grad:
